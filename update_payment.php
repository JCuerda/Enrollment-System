  ...
  if( isset($_POST['btnUpdate']) ){
    ...
    if( $fName == '' ||  $lastName  == '' || $referenceCode == '' || $date== '' ||  $bdoBranch == ''|| $payment_amount == '') {
          ...
          $query = "UPDATE icc_prac3.tbl_payment
                    SET tbl_payment.depositor_fname = '$fName',
                    tbl_payment.depositor_Lname = '$lastName',
                    tbl_payment.reference_code = '$referenceCode',
                    tbl_payment.payment_amount = '$payment_amount',
                    tbl_payment.date = '$date',
                    tbl_payment.bdo_branch = '$bdoBranch'
          ...
          if(!$result){
            ...
            header('Location: profile.php?alert=update_success');
          }
      ...
      $query ="SELECT * FROM icc_prac3.tbl_payment 
              WHERE tbl_payment.studentNo = '$loggedUser' 
              AND tbl_payment.payment_status = 'Rejected' 
              OR tbl_payment.studentNo = '$loggedUser' 
              AND tbl_payment.payment_status = 'Not Approve'";

      $result = mysqli_query($conn,$query);

      if( mysqli_num_rows($result) == 0 ){
          header('Location: profile.php?alert=no_records');
      } else {
