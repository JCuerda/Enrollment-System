<?php
  ...
  if( $loggedUser_branch != 'manila' && $loggedUser_branch != null){
  ...
  if(isset( $_GET['alert'] )){
     ...
     if( $_GET['alert'] == 'success' ){
        ...
        $alertMessage = "<div class='row'><div class='col-md-12'><div class='alert alert-danger'>
                              <strong> No records of Payment in our Database! <a class='close' data-dismiss='alert'>&times</a> </strong> 
                          </div></div></div>";
    ...
    $query = "SELECT tbl_payment_manila.payment_status
              FROM icc_prac3.tbl_payment_manila
              WHERE tbl_payment_manila.studentNo = '$loggedUser'
              AND tbl_payment_manila.status = 'new'";
    ...
    if( $payment_status == 'Not Approve' || $payment_status == 'Rejected') {
      header('Location: profile_manila.php?alert=no_forms');
    } else {
      header('Location:enrollment_form_manila.php');
    }
  ...
  if( isset($_POST['btn_enroll']) ){
      ...
      $query = "SELECT tbl_grades_manila.studentNo , 
                COUNT(tbl_grades_manila.remarks),
                tbl_studentinfo_manila.year_level,
                tbl_studentinfo_manila.semester_num,
                tbl_subject.subject_code
                FROM icc_prac3.tbl_grades_manila,
                icc_prac3.tbl_studentinfo_manila,
                icc_prac3.tbl_subject
                WHERE tbl_subject.subject_code = tbl_grades_manila.subject_code
                ...
                AND tbl_grades_manila.remarks != '$flag'
                AND tbl_studentinfo_manila.studentNo = tbl_grades_manila.studentNo";
      ...
      while( $row = mysqli_fetch_array($result) ){
        ...
        $count = $row['COUNT(tbl_grades_manila.remarks)'];
      ...
      if( $count == 0 ){
          ...
          if( $yr_level == 11 || $yr_level == 12 ){
              header('Location: assessment_form_manila_shs.php');
          } else{
              header('Location: assessment_form_manila.php');
          }
  ...
  if( isset($_POST['btn_update']) ){
      header('Location: update_payment_manila.php'); 
  }
               ...
               ?>
                ...
                ?>
                 ...
                 <div class="row"> <!--Sidebar for Image and About-->
                    ...
                    <div class="col-md-9">
                          <?php include('profile_navigation_manila.php'); ?>
