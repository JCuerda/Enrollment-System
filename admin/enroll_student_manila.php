<?php
    ...
    if( isset($_GET['alert']) ){
        if($_GET['alert'] == 'blank_input'){
             ...
             $errorMsg = "<div class='row'><div class='col-md-12'><div class='alert alert-danger'>
                        <strong>Invalid Name! Name must only consist of Letters<a class='close' data-dismiss='alert'>&times</a> </strong> 
                    </div></div></div>";
    ...
    if( isset($_POST['btnSubmit']) ){
      ...
      if( !$newStudent_courseID || !$newStudent_fName ||  !$newStudent_lastName ||  !$newStudent_mName || !$newStudent_address || !$newStudent_age || !$newStudent_gender || !$newStudent_contactNumber || !$newStudent_emailAdd){
          ...
          header('Location: enroll_student_manila.php?alert=blank_input');

      } else if( !ctype_alpha($newStudent_fName) || !ctype_alpha($newStudent_lastName) || !ctype_alpha($newStudent_mName) ){
          header('Location: enroll_student_manila.php?alert=invalid_name');
      } else if( !ctype_digit($newStudent_contactNumber) ){
          header('Location: enroll_student_manila.php?alert=invalid_contact_number');
      } else {
    

      $newStudentinfo_yearLevel = "1";
      $newStudentinfo_semNum = "1";
      $newStudentinfo_studentType = "regular";
      $newStudentinfo_status = "pending";

          $newStudentinfo_query = "INSERT INTO icc_prac3.tbl_studentinfo_manila (
                                  tbl_studentinfo_manila.studentNo, 
                                  tbl_studentinfo_manila.course_id, 
                                  tbl_studentinfo_manila.first_name, 
                                  tbl_studentinfo_manila.last_name, 
                                  tbl_studentinfo_manila.middle_name,
                                  tbl_studentinfo_manila.branch_id, 
                                  tbl_studentinfo_manila.address, 
                                  tbl_studentinfo_manila.age, 
                                  tbl_studentinfo_manila.gender,
                                  tbl_studentinfo_manila.contact_number, 
                                  tbl_studentinfo_manila.email_address, 
                                  tbl_studentinfo_manila.grant_id,
                                  tbl_studentinfo_manila.year_level, 
                                  tbl_studentinfo_manila.semester_num, 
                                  tbl_studentinfo_manila.student_type, 
                                  tbl_studentinfo_manila.status,
                                  tbl_studentinfo_manila.new_old) 
                                  VALUES (
                                  '', 
                                  '$newStudent_courseID', 
                                   '$newStudent_fName',
              ...
              $result = mysqli_query($conn,$curriculum_enrollment_query);
                ...
                if (mysqli_num_rows($result)){
                  ...
                  while( $row = mysqli_fetch_assoc($result) ){
                    ...
                    $enroll_query = "INSERT INTO icc_prac3.tbl_grades_manila (
                                    tbl_grades_manila.studentNo,
                                    tbl_grades_manila.subject_code,
                ...
                header('Location: ../admin/view_student_manila.php?emailAdd=' . $newStudent_emailAdd); 
        }
     }
 }
            ...
            <div class="col-md-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="active"><a href="../admin/choose_branch.php">Choose Branch</a></li>
                         <li class="active"><a href="../admin/enroll_student_manila.php?branch_id=<?php echo $the_branch;?>">Enrollment Form</a></li>
                    </ol> 
            </div>
        </div>
              <?php echo  $errorMsg;  ?>
         ...
         <form class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
           <div class="panel-content">
            ...
            <div class="form-group">
              <label class="col-md-4 control-label" for="lastSchool">Course :</label>  
              <div class="col-md-6">
